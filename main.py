...
# ── منطق الباقات (10 آلاف = 15 يوم | البقية = 30 يوم) ─────────────────────────────
PLANS = {
...
def handle_withdraw_menu(call):
    ...
    if not user or user.get("active_plan_price", 0) == 0:
        bot.answer_callback_query(call.id, "⚠️ لا توجد باقة نشطة.", show_alert=True)
        return
    ...
    if user.get("deposit_balance", 0) <= 0:
        text = (
            ...
            f"⚠️ الرصيد القابل للسحب الآن: *0 د\\.ع*\\."
        )
        ...
        bot.send_message(call.message.chat.id, "أرسل المبلغ للسحب:")

# ── وظيفة فحص فتح الأرباح ──────────────────────────────────────────────────────
        ...
        if start_date_str:
            try:
                ...
                if (now - BAGHDAD_TZ.localize(start_dt)).days >= needed:
                    ...
                    bot.send_message(u["user_id"], "🎊 تم فتح رصيدك وإضافته للمحفظة!")
            except: pass
...
if __name__ == "__main__":
    try:
        ...
        scheduler.start()
    except: pass # لمنع خطأ التكرار في ريلواي
